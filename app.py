    ...
    for route_type in ['traoptimal', 'trafast', 'tracomfort']:
        ...
        for i in range(len(locations) - 1):
            ...
            try:
                ...
                segments = match_guide_data_to_segments(guide, segments, grade_value, vehicle_type, load_percent)
...
def route_result_dialog():
    if st.session_state.route_result:
        ...
        for route_type in ['traoptimal', 'trafast', 'tracomfort']:
            ...
            st.divider()
@st.dialog('차종별 구간별 보기', width='large')
def segment_detail_dialog():
    ...
    tabs = st.tabs([f"{rt.upper()}" for rt in route_types])
            ...
            for seg in segments:
                for subseg in seg.get('segments', []):
                    row = {
                        ...
                        'TotalCO₂': f"{subseg.get('segment_co2', 0):.4f}",
